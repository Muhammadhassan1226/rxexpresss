...
export interface admindashboard {
  ...
  savesignature: SignatureResponse | null;
}
...
>(
  ...
  async ({ id }, { dispatch, rejectWithValue }) => {
    try {
      ...
      const params = new URLSearchParams();
      const res = await PRIVATE_API.get(
        `api/Delivery/GetAssignOrdersById?id=${id.toString()}`,
      );
...
export const saveOrderSignature = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "Delivery/SaveOrderSignature",
  async ({ orderId, signature }, { rejectWithValue }) => {
    try {
      const res = await PRIVATE_API.post("api/Delivery/SaveOrderSignature", {
        orderId,
        signature,
      });
      ...
      return rejectWithValue(error.message || "Failed to save signature");
    }
  },
);
...
export const deliverySlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(saveOrderSignature.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(saveOrderSignature.fulfilled, (state, action) => {
        state.loading = false;
        state.error = null;
        state.savesignature = action.payload;
      })
      .addCase(saveOrderSignature.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string;
        state.savesignature = null;
      });
  },
});
