...
import { manhattanOrders } from "@/store/slice/adminslice";
...
const AllManhattan = () => {
  ...
  // Fetch ManhattanOrders for the current page
  const handleFetchOrders = async (reset = false) => {
    ...
    try {
      ...
      console.log("Error fetching ManhattanOrders:", error);
    }
  };
  ...
  const loadMoreOrders = async () => {
    if (
      isFetchingMore ||
      ManhattanOrders.orders.length >= ManhattanOrders.totalOrders
    )
      return;
